...
var Swts, swts_log, swts_track;
if (!this.Swts) {
	// Swts singleton and namespace
	Swts = (function () {
		/************************************************************
		 * Private data
		 ************************************************************/
...
*/
			...
			/*
			 * Send image request to Swts server using GET.
			...
			/*
			 * Returns the URL to call swts.php, 
			...
			function logPageView(customTitle) {
				...
				if(getCookie('swtsguid').length != generateGuid().length){
					setCookie('swtsguid', generateGuid());
				}
			...
			function getClassesRegExp(configClasses, defaultClass) {
				var i, classesRegExp = '(^| )(swts[_-]' + defaultClass;
...
/*
				...
				if (linkPattern.test(className)) {
					// class attribute contains 'swts_link' (or user's override)
					linkType = 'link';
				} else if (downloadPattern.test(className)) {
					// class attribute contains 'swts_download' (or user's override)
					linkType = 'download';
 ...
 */
			...
			return {
				...
				/*
				 * Set download class name (i.e., override default: swts_download)
				 * (deprecated)
				 */
				...
				/*
				 * Set outlink class name (i.e., override default: swts_link)
				 * (deprecated)
				 */
				 ...
				 * When clicked, Swts will log the click automatically.
				 */
		...
		return {
			 ...
			 * Get Tracker
			 */
			getTracker: function (swtsUrl, siteId) {
				return new Tracker(swtsUrl, siteId);
			}
		};
	}());
	 ...
	 * - for legacy swts.js compatibility
	 ************************************************************/

	/*
	 * Swts globals
	 *
	 *   var swts_install_tracker, swts_tracker_pause, swts_download_extensions, swts_hosts_alias, swts_ignore_classes;
	 */
	 ...
	 * Track page visit
	 */
	swts_log = function (documentTitle, siteId, swtsUrl, customData) {

		function getOption(optionName) {
			try {
				return eval('swts_' + optionName);
			} catch (e) { }
		...
		var swtsTracker = Swts.getTracker(swtsUrl, siteId);

		// initializer tracker
		swtsTracker.setDocumentTitle(documentTitle);
		swtsTracker.setCustomData(customData);

		// handle Swts globals
		swtsTracker.setLinkTrackingTimer(getOption('tracker_pause'));
		swtsTracker.setDownloadExtensions(getOption('download_extensions'));
		swtsTracker.setDomains(getOption('hosts_alias'));
		swtsTracker.setIgnoreClasses(getOption('ignore_classes'));
		...
		if (getOption('install_tracker') !== false) {
			 ...
			 */
			swts_track = function (sourceUrl, siteId, swtsUrl, linkType) {
				swtsTracker.setSiteId(siteId);
				swtsTracker.setTrackerUrl(swtsUrl);
				swtsTracker.trackLink(sourceUrl, linkType);
			};
